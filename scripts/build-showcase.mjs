...
const groupNames={apps:'Aplicaciones',herramientas:'Herramientas',atlassian:'Integraciones Atlassian',proyectos:'Proyectos'};
const categoryOf=e=>['deleteguard','broken-link-guard','leaver-guard'].includes(e.slug)?'atlassian':(e.category||e.group);
...
const productPreviews={
  tutest:{src:'/assets/tutest-preview.svg',alt:'Vista previa del informe de auditoría técnica de TuTest'},
  tunegocio:{src:'/assets/tunegocio-preview.svg',alt:'Vista previa de una web creada con TuNegocio'}
};
const previewImg=(preview,eager=false)=>`<img src="${preview.src}" alt="${esc(preview.alt)}" width="960" height="620" ${eager?'fetchpriority="high"':'loading="lazy"'} decoding="async">`;
const productVisual=(e,eager=false)=>productPreviews[e.slug]?previewImg(productPreviews[e.slug],eager):e.visual?`<div class="product-graphic graphic-${e.slug}"><span class="graphic-code">${esc(e.code)}</span><strong>${esc(e.name)}</strong><ol>${e.visual.map(v=>`<li>${esc(v)}</li>`).join('')}</ol><small>${e.slug==='tutest'?'Audit Intelligence V5':'Web para pequeños negocios'}</small></div>`:img(e.image,e.alt,eager)+(e.previewHome?img(e.previewHome,'Pantalla real de inicio de Fivaki en Android, con una cuenta de prueba.',eager):'');
const nav=`<nav aria-label="Principal"><a href="/#productos">Aplicaciones</a><a href="/#herramientas">Herramientas</a><a href="/#atlassian">Atlassian</a><a href="/#proyectos">Proyectos</a><a class="nav-portfolio" href="${portfolio}">Quién soy <span aria-hidden="true">↗</span></a><a href="/soporte/">Soporte</a></nav>`;
...
function page(title,description,route,body,cover){
  return `<!doctype html>
<html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title)} — NoeApps</title><meta name="description" content="${esc(description)}"><meta name="robots" content="index,follow"><meta name="theme-color" content="#102b49"><link rel="icon" type="image/svg+xml" href="/assets/favicon.svg"><link rel="canonical" href="https://noeapps.com${route}"><meta property="og:title" content="${esc(title)} — NoeApps"><meta property="og:description" content="${esc(description)}"><meta property="og:type" content="website"><meta property="og:site_name" content="NoeApps"><meta property="og:locale" content="es_ES"><meta name="twitter:card" content="summary"><meta name="twitter:title" content="${esc(title)} — NoeApps"><meta name="twitter:description" content="${esc(description)}"><meta property="og:url" content="https://noeapps.com${route}">${cover?`<meta property="og:image" content="https://noeapps.com${asset(cover)}">`:''}<link rel="stylesheet" href="/styles.css"><link rel="stylesheet" href="/catalog.css"><link rel="stylesheet" href="/presentation.css"><link rel="stylesheet" href="/brand-refresh.css"></head><body><a class="skip-link" href="#contenido">Ir al contenido</a>${header}<main id="contenido" class="wrap">${body}</main>${footer}</body></html>\n`;
}
...
function section(group,id,number,title,description,intro=''){
  return `<section class="catalog-section" id="${id}"><div class="section-heading"><div><p class="eyebrow">${number} / ${groupNames[group].toUpperCase()}</p><h2>${title}</h2></div><p>${description}</p></div>${intro}<div class="catalog-grid grid-${group}">${entries.filter(e=>categoryOf(e)===group).map(card).join('')}</div></section>`;
}
const hero=`<section class="visual-hero" id="apps"><div><p class="eyebrow">NOEAPPS / APLICACIONES Y PROYECTOS</p><h1>Encuentra software<br>que te resulte <em>útil.</em></h1><p class="hub-lead">Explora todas las aplicaciones, herramientas e integraciones de NoeApps. Descubre para qué sirve cada una, en qué estado está y cómo probarla.</p><div class="hero-actions"><a class="button button-dark" href="#catalogo">Explorar el catálogo <span aria-hidden="true">↓</span></a><a class="text-link" href="${portfolio}">Conoce a su creador</a></div></div><div class="hero-mosaic" aria-label="Una selección de productos NoeApps"><a class="mosaic-main" href="/apps/forgeops/"><span><b>ForgeOps</b><small>Demo de mantenimiento industrial</small></span>${img('forgeops-dashboard.webp','Panel real de demostración de ForgeOps.',true)}</a><a class="mosaic-product mosaic-fivaki" href="/apps/fivaki/">${img('fivaki-icon-original.png','Icono original completo de Fivaki.')}<span>Fivaki</span></a><a class="mosaic-product mosaic-preview" href="/apps/tutest/">${previewImg(productPreviews.tutest)}<span>TuTest</span></a><a class="mosaic-product mosaic-preview" href="/apps/tunegocio/">${previewImg(productPreviews.tunegocio)}<span>TuNegocio</span></a></div></section><nav class="catalog-jump" id="catalogo" aria-label="Secciones del catálogo">${[['apps','productos'],['herramientas','herramientas'],['atlassian','atlassian'],['proyectos','proyectos']].map(([group,id])=>`<a href="#${id}"><strong>${String(entries.filter(e=>categoryOf(e)===group).length).padStart(2,'0')}</strong><span>${groupNames[group]}</span><span class="jump-arrow" aria-hidden="true">↗</span></a>`).join('')}</nav>`;
writePage('/',page('Aplicaciones para trabajar, crear y avanzar','Explora 19 productos y proyectos: TuTest, TuNegocio, ForgeOps, Fivaki, herramientas, integraciones Jira y Confluence, medios digitales y laboratorios.','/',hero+`<section class="catalog-section" id="destacados"><div class="section-heading"><div><p class="eyebrow">DESTACADOS</p><h2>Destacados para empezar.</h2></div><p>Analiza una web, explora el mantenimiento de tu planta o crea la presencia de tu negocio. El catálogo completo continúa debajo.</p></div><div class="featured-links">${['tutest','forgeops','tunegocio'].map(slug=>{const e=entries.find(x=>x.slug===slug);return `<article><p class="eyebrow">${esc(e.label)}</p><h3><a href="${urlOf(e)}">${esc(e.name)}</a></h3><p>${esc(e.description)}</p><a class="button button-dark" href="${esc(accessOf(e))}">${esc(e.cta)}</a><a class="text-link" href="${urlOf(e)}">Conocer ${esc(e.name)}</a></article>`}).join('')}</div></section>`+
section('apps','productos','01','Cada aplicación resuelve algo distinto.','Organiza el mantenimiento, cuida tus hábitos, gestiona tu negocio o crea su presencia web.')+
...
section('atlassian','atlassian','03','Integraciones para Jira y Confluence.','Administración, contexto y cambios controlados dentro de tu entorno Atlassian.')+
section('proyectos','proyectos','04','Más allá de una sola aplicación.','Medios especializados, experiencias digitales y laboratorios de software e industria. Visita las webs o conoce cada proyecto.')+
...
for(const e of entries){
  ...
  const body=`<nav class="breadcrumb" aria-label="Ruta de navegación"><a href="/">NoeApps</a><span aria-hidden="true">/</span><a href="/#${anchorOf(e)}">${groupNames[categoryOf(e)]}</a><span aria-hidden="true">/</span><span aria-current="page">${esc(e.name)}</span></nav>
...
${e.coverage?`<section class="story-section"><div class="story-heading"><p class="eyebrow">COBERTURA TÉCNICA</p><h2>Una visión conjunta de tu web.</h2><p>Auditoría profunda sobre la entrada, una muestra interna y la infraestructura pública. El informe muestra qué ha podido evaluar.</p></div><div class="benefit-grid">${e.coverage.map(([title,copy])=>`<article class="benefit-item"><h3>${esc(title)}</h3><p>${esc(copy)}</p></article>`).join('')}</div></section>`:''}${gallery}<section class="story-section faq-layout"><div class="faq-list"><h2>Preguntas frecuentes</h2>${e.faq.map(([question,answer])=>`<details><summary>${esc(question)}</summary><p>${esc(answer)}</p></details>`).join('')}</div><aside class="story-access"><p class="eyebrow">EMPIEZA POR AQUÍ</p><h2>${esc(e.status)}</h2><p>${esc(e.access)}</p><a class="button button-light" href="${esc(accessOf(e))}">${esc(e.cta)} <span aria-hidden="true">↗</span></a>${!e.url?'<p class="small-copy">Cuéntanos tu caso por correo para concretar una presentación o las condiciones de acceso.</p>':`<p class="small-copy">¿Quieres comentar tu caso? <a href="${contact(e.name)}">Escríbenos</a>.</p>`}</aside></section>${docs}
<section class="more-catalog"><h2>También puedes descubrir</h2><div>${entries.filter(x=>categoryOf(x)===categoryOf(e)&&x.slug!==e.slug).slice(0,3).map(x=>`<a href="${urlOf(x)}">${esc(x.name)} <span aria-hidden="true">↗</span></a>`).join('')}</div><a class="text-link" href="/#${anchorOf(e)}">Ver ${groupNames[categoryOf(e)].toLowerCase()}</a></section>`;
  let html=page(e.name,e.description,urlOf(e),body,e.image);
  if(e.slug==='tutest')html=html.replace('</head>',`<script type="application/ld+json">${JSON.stringify({'@context':'https://schema.org','@type':'SoftwareApplication',name:'TuTest',url:e.url,applicationCategory:'DeveloperApplication',operatingSystem:'Web',description:e.description,offers:{'@type':'Offer',price:'0',priceCurrency:'EUR'},publisher:{'@type':'Organization',name:'NoeApps',url:'https://noeapps.com'}})}</script></head>`);
  writePage(urlOf(e),html);
}

const supportFile=path.join(root,'soporte/index.html');
let support=fs.readFileSync(supportFile,'utf8').replace(/<section id="catalog-support">[\s\S]*?<\/section>/,'');
support=support.replace('</article>',`<section id="catalog-support"><h2>Todo el catálogo, en un mismo lugar.</h2><div class="link-grid">${entries.map(e=>`<a class="link-card" href="${urlOf(e)}"><h3>${esc(e.name)}</h3><p>${esc(e.label)} · ${esc(e.status)}</p></a>`).join('')}</div></section></article>`);
...
// Keep the previously published compact aliases reachable.
for(const [alias,canonical] of [['brokenlinkguard','broken-link-guard'],['leaverguard','leaver-guard']])writePage(`/apps/${alias}/`,fs.readFileSync(path.join(root,'apps',canonical,'index.html'),'utf8'));
